...
const BlogDetailsCard = ({ blog }: { blog: TBlog }) => {
  ...
  return (
    <motion.div
      className="md:w-2/3 bg-white shadow-lg rounded-lg mx-auto p-6"
      initial={{ opacity: 0, y: 50 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.6, ease: "easeOut" }}
    >
      <motion.p
        className="flex items-center justify-center mx-auto text-teal-500 bg-teal-100 w-fit px-3 py-1 rounded-full"
      ...
      <motion.div
        className="flex items-center justify-center bg-gray-100 mb-5 py-2 rounded-lg gap-2"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
